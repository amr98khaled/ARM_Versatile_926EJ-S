...
#define UDS 0x04                // status register
#define UFR 0x18                // Flag register
#define CNTL 0x2C               // Line control register
...
int uart_init()
{
    ...
    {   // uart0 to uart2 are adjacent
        ...
        *(up->base+CNTL) &= ~0x10; // disable UART FIFO (character mode) , the FIFOs become 1-byte-deep holding registers.
        *(up->base+IMSC) |= 0x30;   // set transmit & receive interrupt masks
...
int uputc(UART *up, char c)
{
    ...
    while( *(up->base+UFR) & 0x20 ); // loop while FR=TXF (transmit fifo full)
    *(up->base+UDR) = (int)c;        // write c to DR
